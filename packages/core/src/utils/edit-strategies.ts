...
export function applyStrategicReplacement(
  content: string,
  oldString: string,
  newString: string,
  replaceAll = false,
  ...
  for (const replacer of [
    ...
    for (const search of replacer(normalizedContent, normalizedOldString)) {
      ...
      if (replaceAll) {
        ...
        return {
          newContent,
          occurrences: normalizedContent.split(search).length - 1,
          finalOldString: search, // This is an approximation if replaceAll
      ...
      return {
        newContent,
        occurrences: 1,
        finalOldString: search,
        finalNewString: normalizedNewString,
      };
    }
  }

  // Return null result if not found, let caller handle error
  return {
    newContent: content, // return original content
    occurrences: 0,
    finalOldString: oldString,
    finalNewString: newString,
  };
